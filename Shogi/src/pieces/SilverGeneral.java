...
public class SilverGeneral implements Piece {

	private int curX, curY, dir;
	public final boolean PROMOTED = false;
	
	public SilverGeneral(int startX, int startY){
		curX = startX;
		curY = startY;
		if(curX < 3)
			dir = 1;
	...
	public boolean moveIsLegal(int newX, int newY) {
		...
		// Silver General can move one space diagonally
		// or one space directly Forward
		
		// One Space Diagonally
		
		if(Math.abs(newY - curY) == 1 && Math.abs(newX - curX) == 1)
			return true;
	...
	public int getId(boolean japanese) {
		if(japanese)
			return R.drawable.silverjp;
		return R.drawable.silvereng;
	}

	@Override
	public Piece flip() {
		return new PromotedSilver(this);
	}
