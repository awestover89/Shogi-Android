...
public class PromotedKnight implements Piece {

	private int curX, curY, dir;
	public final boolean PROMOTED = true;
	
	public PromotedKnight(int startX, int startY){
		curX = startX;
		curY = startY;
		if(curX < 3)
			dir = 1;
	...
	public boolean moveIsLegal(int newX, int newY) {
		...
		// Promoted Knight can move one space Up, Down, Left or Right
	...
	public Piece deepCopy() {
		return new PromotedKnight(this);
	}
	...
	public int getId(boolean japanese) {
		if(japanese)
			return R.drawable.knightpromjp;
		return R.drawable.knightpromeng;
	}
